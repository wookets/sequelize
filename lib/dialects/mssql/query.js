  ...
  Query.prototype.run = function(sql) {
    ...
    var request = new Request(this.sql, function(err, rowCount) {
      ...
      if (err) {
        ...
        if (self.sql.indexOf(QueryGenerator.describeTableQuery('table').slice(0, 20)) === 0) {
          results = {}

          self.rows.forEach(function(_result) {
            var type = _result.Type.toUpperCase()

            if (String(_result.Length || '').trim() !== '') {
              type = type + "(" + _result.Length + ")"
            }

            results[_result.Field] = {
              type:         type,
              allowNull:    (_result.Null === 'YES'),
              defaultValue: _result.Default
            }
          })
        } else if (isShowTableQuery.call(self)) {
